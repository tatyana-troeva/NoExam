...
function getSBLIPrices($data){
    ...
    if (in_array($data['state'], $DISALLOWED_STATES['sbli'], true)) {
        return [];
    }
...
?>
    ...
    <div id="form-area">
        ...
        $na_rate = getNARate(array ('Height_ft' => $Height_ft, 'Height_in' => $Height_in, 'sex' => $Sex,
            'weight' => $weight, 'smoker' => $Smoker,));

        //get sbli prices
        $sbli_prices = getSBLIPrices(array('sex' => $Sex, 'type' => $sbli_rate, 'age' => $age, 'state' => $State));
...
//        $terms = array(10, 15, 20, 25, 30);
        $terms = array(10, 15, 20,  30);

        // build price array
        $prices = array('sbli' => array(), 'sagicor' => array(), 'na' => array());
        $sbli_band = getSBLIBand($FaceAmount);
        $na_band = getNABand($FaceAmount);
        ...
        foreach ($na_prices as &$na_price):
            $na_price ->annual = getNAPrice($na_price, $na_band, 'year');
            $na_price ->monthly = getNAPrice($na_price, $na_band, 'month');
            $prices['na'][(string)$na_price->term] = $na_price;
        endforeach;

        // this is for 10 year: sagicor
        if (!in_array($State, $DISALLOWED_STATES['sagicor'], true)) {
                                        ...
                                        <div class="resultcontentcol apply">
                                            ...
                                            <?php elseif ($product_key == 'na'): ?>
                                                <form action="<?php echo $url; ?>" method=post style="margin-bottom: 0px;">
                                                    ...
                                                    <input type=hidden name="fpremium" value="<?php echo number_format($prices[$product_key][$term]->monthly, 2); ?>">
                                                    <input type=hidden name="fannualpremium" value = "<?php echo number_format($prices[$product_key][$term]->annual, 2);?>">
                                                    <input type=hidden name="fcompany" value="NA">
                                                    <input type=hidden name="fproduct" value="NA Term <?php echo $term;?>">
                                                    <!--undefined fields-->
                                                    <input type=hidden name="fHealth" value="<?php echo $Health; ?>">
                                                    <input type=hidden name="fgclid_field" value="<?php echo $gclid; ?>">
                                                    <input type=hidden name="fbing_field" value="<?php echo $bingid; ?>">
